...
## 1. Test Strategy
The testing strategy for TokTickIT Lab 2 guarantees complete functional correctness, multi-user data isolation, boundary validation, and responsive UI behavior through five distinct testing layers:
...
2. **API Integration Testing (Supertest + Vitest):** Verifies all backend endpoints, HTTP status codes, payload structures, database persistence, transaction rollback, and ownership enforcement.
3. **Frontend UI Component Testing (React Testing Library + Vitest):** Verifies screen rendering, component state transitions (idle, busy, success, error, empty), form validation messages, and user interactions with mocked API services.
