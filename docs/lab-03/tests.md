...
| T-08 | AC-08 claim/reassign/priority/complete status matrix authorization | API + UI + security + E2E | staff-queue.api.test.ts; staff-ticket-detail.api.test.ts; StaffTicketDetail.test.tsx; staff-ticket-flow.spec.ts |
...
- server/tests/lab-03/auth.api.test.ts: valid/invalid login, inactive-account response after password verification, logout, session, CSRF, password policy, first-login gate, expiry.
...
- server/tests/lab-03/staff-queue.api.test.ts: queue search/filter/sort/pagination, invalid query behavior, claim race, reassignment to active IT Staff/Admin, and IT priority.
