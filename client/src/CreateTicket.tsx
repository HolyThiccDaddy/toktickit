...
import { ApiError, createTicket, getReferenceData, type Category, type CreatedTicket, type RelatedSystem, type UserSummary } from "./api.js";

const maxFileSize = 5_242_880;
const allowedExtensions = [".jpg", ".jpeg", ".png", ".webp", ".pdf"];
const allowedTypes = ["image/jpeg", "image/png", "image/webp", "application/pdf"];

export default function CreateTicket({ user, onCancel }: { user: UserSummary; onCancel: () => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try { setCreated(await createTicket(data)); } catch (error) {
  ...
  return <form className="card shadow-sm p-4" onSubmit={submit} noValidate><h1 className="h3 text-zen">Create Ticket</h1><p className="text-muted">Fields marked <span className="required-marker">*</span> are required.</p>
    {loadingReferences && <p>Loading ticket reference data...</p>}{referenceError && <div className="alert alert-danger" role="alert">{referenceError}<button type="button" className="btn btn-outline-danger d-block mt-2" onClick={loadReferences}>Retry</button></div>}
    <div className="row g-3"><div className="col-md-6"><label htmlFor="ticket-date" className="form-label">Ticket Date</label><input id="ticket-date" className="form-control" value="Assigned on submission" readOnly /></div><div className="col-md-6"><label htmlFor="requester-display" className="form-label">Requester</label><input id="requester-display" className="form-control" value={displayName} readOnly /></div>
    <div className="col-md-6"><label htmlFor="category" className="form-label">Category {required}</label><select id="category" aria-invalid={Boolean(errors.categoryId)} aria-describedby={errors.categoryId ? "category-error" : undefined} className={`form-select ${errors.categoryId ? "is-invalid" : ""}`} value={categoryId} onChange={(e) => setCategoryId(e.target.value)} disabled={loadingReferences}><option value="">Select category</option>{categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>{errors.categoryId && <div id="category-error" className="invalid-feedback">{errors.categoryId}</div>}</div>
    <div className="col-md-6"><label htmlFor="system" className="form-label">Related System {required}</label><select id="system" aria-invalid={Boolean(errors.relatedSystemId)} aria-describedby={errors.relatedSystemId ? "system-error" : undefined} className={`form-select ${errors.relatedSystemId ? "is-invalid" : ""}`} value={relatedSystemId} onChange={(e) => setRelatedSystemId(e.target.value)} disabled={loadingReferences}><option value="">Select related system</option>{systems.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>{errors.relatedSystemId && <div id="system-error" className="invalid-feedback">{errors.relatedSystemId}</div>}</div>
    <div className="col-12"><label htmlFor="summary" className="form-label">Ticket Summary {required}</label><input id="summary" aria-invalid={Boolean(errors.summary)} aria-describedby={errors.summary ? "summary-error" : undefined} className={`form-control ${errors.summary ? "is-invalid" : ""}`} maxLength={150} value={summary} onChange={(e) => setSummary(e.target.value)} />{errors.summary && <div id="summary-error" className="invalid-feedback">{errors.summary}</div>}</div>
    <div className="col-12"><label htmlFor="description" className="form-label">Description {required}</label><textarea id="description" aria-invalid={Boolean(errors.description)} aria-describedby={errors.description ? "description-error" : undefined} className={`form-control ${errors.description ? "is-invalid" : ""}`} rows={5} maxLength={2000} value={description} onChange={(e) => setDescription(e.target.value)} />{errors.description && <div id="description-error" className="invalid-feedback">{errors.description}</div>}</div>
