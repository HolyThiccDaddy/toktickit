...
export default function CreateTicket({ requester, onCancel }: { requester: Requester; onCancel: () => void }) {
  ...
  if (created) return <section className="alert alert-success" role="status"><h2 className="h4">Ticket created successfully</h2><p>Official Ticket Number: <strong>{created.ticketNumber}</strong></p><button className="btn btn-outline-zen" onClick={onCancel}>Back</button></section>;
  ...
  return <form className="card shadow-sm p-4" onSubmit={submit} noValidate><h1 className="h3 text-zen">Create Ticket</h1><p className="text-muted">Fields marked <span className="required-marker">*</span> are required.</p>
    ...
    <div className="row g-3"><div className="col-md-6"><label className="form-label">Ticket Date</label><input className="form-control" value={new Date().toLocaleDateString()} readOnly /></div><div className="col-md-6"><label className="form-label">Requester</label><input className="form-control" value={requester.name} readOnly /></div>
