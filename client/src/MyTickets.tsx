...
export default function MyTickets({ requester, onCreate, onView }: { requester: Requester; onCreate: () => void; onView?: (ticketId: number) => void }) {
  ...
  return <section className="my-tickets" aria-labelledby="my-tickets-title">
    ...
    {!loading && !error && tickets.length > 0 && <>
      ...
      </tr></thead><tbody>{tickets.map((ticket) => <tr key={ticket.id}><td>{ticket.ticketNumber}</td><td>{new Date(ticket.createdAt).toLocaleDateString()}</td><td>{ticket.summary}</td><td>{ticket.category.name}</td><td><span className={`priority-badge priority-${ticket.requestedPriority.toLowerCase()}`}>{ticket.requestedPriority}</span></td><td><span className="status-badge">{ticket.currentStatus}</span></td><td><button className="btn btn-sm btn-outline-zen" type="button" onClick={() => onView?.(ticket.id)}>View</button></td></tr>)}</tbody></table></div>
      <div className="ticket-card-list" data-testid="ticket-card-list">{tickets.map((ticket) => <article className="card shadow-sm p-3" key={ticket.id}><div className="d-flex justify-content-between gap-2"><strong>{ticket.ticketNumber}</strong><span className="status-badge">{ticket.currentStatus}</span></div><h2 className="h6 my-2">{ticket.summary}</h2><div className="d-flex flex-wrap align-items-center gap-2 small text-muted"><span className="category-badge">{ticket.category.name}</span><span>{new Date(ticket.createdAt).toLocaleDateString()}</span></div><span className={`priority-badge priority-${ticket.requestedPriority.toLowerCase()} align-self-start mt-2`}>{ticket.requestedPriority}</span><button className="btn btn-sm btn-outline-zen mt-3" type="button" onClick={() => onView?.(ticket.id)}>View ticket</button></article>)}</div>
