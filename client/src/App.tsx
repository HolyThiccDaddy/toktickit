...
function RequesterWorkspace({ requester, onChangeRequester }: { requester: Requester; onChangeRequester: () => void }) {
  ...
  const navigation = <nav className="d-flex gap-2" aria-label="Primary navigation"><button className={`zen-nav-button px-2 py-1 ${view === "system" ? "active" : ""}`} type="button" onClick={() => setView("system")}>Home</button><button className={`zen-nav-button px-2 py-1 ${view === "tickets" || view === "detail" ? "active" : ""}`} type="button" onClick={() => setView("tickets")}>My Tickets</button><button className={`zen-nav-button px-2 py-1 ${view === "create" ? "active" : ""}`} type="button" onClick={() => setView("create")}>+ Create Ticket</button></nav>;
  ...
  if (view === "tickets") return <>{header}<main className="container py-4"><MyTickets requester={requester} onCreate={() => setView("create")} onView={(ticketId) => { setSelectedTicketId(ticketId); setView("detail"); }} /></main></>;
  if (view === "detail" && selectedTicketId !== null) return <>{header}<main className="container py-4"><TicketDetail requester={requester} ticketId={selectedTicketId} onBack={() => setView("tickets")} /></main></>;
