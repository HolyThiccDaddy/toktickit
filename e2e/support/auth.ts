...
export async function signInAsStaff(page: any, account: E2EAccount) {
  await page.goto("/");
  const loginHeading = page.getByRole("heading", { name: "Sign in to IT Service Desk" });
  const changePasswordHeading = page.getByRole("heading", { name: "Change your password" });
  const queueHeading = page.getByRole("heading", { name: "Ticket Queue" });

  async function waitForStaffShell() {
    try {
      await expect(queueHeading.or(changePasswordHeading)).toBeVisible({ timeout: 5_000 });
      return true;
    } catch {
      return false;
    }
  ...
  await expect(queueHeading.or(changePasswordHeading)).toBeVisible();
  if (await changePasswordHeading.isVisible().catch(() => false)) {
    const nextPassword = account.changedPassword;
    await page.getByLabel("Current password").fill(account.currentPassword ?? account.initialPassword);
    await page.getByLabel("New password", { exact: true }).fill(nextPassword);
